...
export function PhotoToVideoView({ locale }: { locale: Locale }) {
  const feature = getFeature("photo-to-video", locale)!;
  const content = getContent(locale);
  const t = getDictionary(locale).toolPages.photoToVideo;
      ...
      <Section>
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="mx-auto w-full max-w-[340px]">
            <Screenshot shot={content.shots.breathe} priority width={340} />
          </div>
        </div>
      </Section>

      <Section className="bg-surface">
        <SectionHeading eyebrow={c.howItWorksEyebrow} title={t.howTitle} />
