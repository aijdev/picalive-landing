...
export function Button({
  ...
  // Same-page anchors (e.g. "#pro") are always relative to the current page,
  // regardless of locale prefix — never route them through localizedPath.
  const resolvedHref = href.startsWith("#") ? href : localizedPath(href, locale);

  return (
    <Link href={resolvedHref} aria-label={ariaLabel} className={classes}>
      {children}
    </Link>
  );
}
